 ...
 * @author DELL
 */
@WebServlet(name = "BuyConfirmButton", urlPatterns = {"/confirm"})
public class BuyConfirmButton extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            ...
            if (o.getName().equals("id")) {
                ...
                for (String i : arrID) {
                    ...
                    if (dao.getProductById(i) != null) {
                        listBuy.add(dao.getProductById(i));
                    }
                }
                break;
            }
        }
        //xu ly list san pham cua minh
        ...
        if (request.getParameter("code") != null) {
            ...
            if (code.equals("BẠN CỦA ĐẠT")) {
                totalPay = totalPay * 10 / 100;// giảm giá còn 10% tổng giá trị đơn hàng
            }
        }
        
//        if (totalPay == 0) {
//            response.sendRedirect("NoticeCart.jsp");
//        }
        
        //cập nhật lại số lượng đã bán được cho sản phẩm
        loadDAO ld =new loadDAO();
        for (Product l : listBuy) {
            Product p=ld.getProductByID(l.getId());
            int newBuyAmout=l.getAmoutBuy()+l.getAmount();
            ld.updateAmountBuy(newBuyAmout, l.getId());
        }
        ...
        String detail="";
        for (Product l : listBuy) {
            detail+="|"+l.getName()+"x"+l.getAmount()+"x"+l.getPrice();
        }
        String status="Chờ xác nhận";
        String deliveryAddress=request.getParameter("deliveryAddress");
        String payment=request.getParameter("payment");
        ld.insertIntoOrderDetail(cusName, detail, totalPay, status, deliveryAddress, payment);
